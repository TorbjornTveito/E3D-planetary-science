...
def datetime_to_utc_sec(dt):
    return float(calendar.timegm(dt.timetuple()))

def date2unix(year,month,day,hour,minute,second):
    t = datetime.datetime(year, month, day, hour, minute, second,tzinfo=UTC())
    return(datetime_to_utc_sec(t))#float(t.strftime("%s")))#timestamp())#time.mktime(t.timetuple()))
